...
public class ControllerAdvice {
    ...
    /**
     * O handler usado para tratar a listagem de pedido vazio.
     * @param ex (PedidoNaoExisteException)
     * @param request (WebRequest)
     * @return HTTPResponse 404 com a mensagem de erro (String)
     */
    @ExceptionHandler(PedidoNaoExisteException.class)
    protected ResponseEntity<Object> handlePedidoNaoExisteException(PedidoNaoExisteException ex, WebRequest request) {
        return ResponseEntity.badRequest().body("Não há pedido para esse cliente.");
    }
